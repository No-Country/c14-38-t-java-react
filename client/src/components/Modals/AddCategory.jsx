      ...
      <div className="fixed inset-0 bg-black bg-opacity-30 backdrop-blur-sm flex justify-center items-center">
          <div className="bg-gray-100 sm:bg-white p-4 sm:pt-9 sm:pb-8 sm:rounded-xl flex flex-col sm:justify-center sm:align-baseline gap-5 absolute sm:relative w-full sm:w-auto h-full sm:h-auto overflow-auto sm:overflow-visible">
              <button className="absolute sm:top-0 sm:right-0 sm:my-4 mt-0.5 ml-3 sm:mx-8" onClick={() => setIsCategoryModal(false)}>
                  ...
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 sm:hidden text-custom-black">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M10.5 19.5L3 12m0 0l7.5-7.5M3 12h18" />
                  </svg>
              </button>
              <h3 className="flex font-[Roboto] text-xl font-medium items-start mx-12 sm:mx-16 text-custom-black">Categorías</h3>
              <div className="flex flex-col gap-5 mx-4 sm:ml-0 sm:mr-0">
                  ...
                  <div className="h-screen sm:h-36 sm:ml-14 sm:mr-16 overflow-auto">
                    <ul className="flex flex-col gap-0.5 bg-white">
                        {selectCategory && (
                                category.map((item) => 
                                        <li key={item.id} className={`bg-gray-200 text-custom-black py-2.5 px-8 cursor-pointer border border-solid  ${selectedCategory === item.name ? 'border-custom-blue' : ''}`} onClick={() => handleItemClick(item.name)}>{item.name}</li>
                                    )
                        )}
                    </ul>
                  </div>
